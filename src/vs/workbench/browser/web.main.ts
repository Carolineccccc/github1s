...
class BrowserMain extends Disposable {
	...
	private registerListeners(
		...
		this._register(
			addDisposableListener(viewport, EventType.RESIZE, () => {
				logService.trace(
					`web.main#${
						isIOS && window.visualViewport ? 'visualViewport' : 'window'
					}Resize`
				);
				workbench.layout();
			})
		...
		this._register(
			addDisposableListener(this.domElement, EventType.CONTEXT_MENU, (e) =>
		...
		this._register(
			addDisposableListener(this.domElement, EventType.DROP, (e) =>
				EventHelper.stop(e, true)
			)
		);
		...
		// Workbench Lifecycle
		this._register(
			workbench.onBeforeShutdown((event) => {
		...
		// Fullscreen (Browser)
		[EventType.FULLSCREEN_CHANGE, EventType.WK_FULLSCREEN_CHANGE].forEach(
			(event) => {
				this._register(
					addDisposableListener(document, event, () =>
						setFullscreen(!!detectFullscreen())
					)
				);
			}
	...
	private async initServices(): Promise<{
		...
		const environmentService = new BrowserWorkbenchEnvironmentService(
			{ workspaceId: payload.id, logsPath, ...this.configuration },
			productService
		);
		...
		const connectionToken =
			...
			this.getCookieValue('vscode-tkn');
		...
		const remoteAgentService = this._register(
			new RemoteAgentService(
				this.configuration.webSocketFactory,
				environmentService,
				productService,
				remoteAuthorityResolverService,
				signService,
	...
	private async registerFileSystemProviders(
		...
		(async () => {
			...
			try {
				indexedDBLogProvider = await indexedDB.createFileSystemProvider(
					logsPath.scheme,
					INDEXEDDB_LOGS_OBJECT_STORE
				);
			} catch (error) {
			...
			logService.logger = new MultiplexLogService(
				coalesce([
					new ConsoleLogService(logService.getLevel()),
					new FileLogService(
						'window',
						environmentService.logFile,
						logService.getLevel(),
						fileService
					),
					...
					!!environmentService.extensionTestsLocationURI
						? new ConsoleLogInAutomationService(logService.getLevel())
						: undefined,
				])
			);
		})();
		...
		if (connection) {
			...
			const remoteFileSystemProvider = this._register(
				new RemoteFileSystemProvider(remoteAgentService)
			);
			fileService.registerProvider(
		...
		try {
			indexedDBUserDataProvider = await indexedDB.createFileSystemProvider(
				Schemas.userData,
				INDEXEDDB_USERDATA_OBJECT_STORE
			);
		} catch (error) {
				...
				class ResetUserDataAction extends Action2 {
					constructor() {
						super({
							...
							title: {
								original: 'Reset User Data',
								value: localize('reset', 'Reset User Data'),
							},
		...
		fileService.registerProvider(
			Schemas.userData,
			indexedDBUserDataProvider || new InMemoryFileSystemProvider()
		);
	}
